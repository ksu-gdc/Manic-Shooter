...
namespace TextPackage
{
    public class FontRenderer
    {
        ...
        {
            InitRenderer(fontFile, fontTexture);
        }

        public FontRenderer(ContentManager contentManager, string fontName)
        {
            InitRenderer(FontLoader.Load(Path.Combine(contentManager.RootDirectory, fontName + ".fnt")),
                contentManager.Load<Texture2D>(fontName + "_0.png"));
        }

        private void InitRenderer(FontFile fontFile, Texture2D fontTexture)
        {
            _fontFile = fontFile;
        ...
        public void DrawText(SpriteBatch spriteBatch, int x, int y, string text)
        {
            ...
            foreach (char c in text)
            {
                ...
                {
                    dx = x;
                    dy += _fontFile.Info.Size;
                }
                else if (c == '\t')
                {
                    _characterMap.TryGetValue(' ', out fc);
                    dx += fc.XAdvance * 5;
                }
            }
        }
